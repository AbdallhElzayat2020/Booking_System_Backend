        ...
        <div class="card-body">
            ...
            <form action="{{ route('admin.stripe-settings.update') }}" method="post">
                ...
                <div class="row">
                        ...
                        <div class="form-group">
                            ...
                            <select name="stripe_country" id="stripe_country" class="form-control select2">
                                ...
                                @foreach (config('countries') as $code => $country)
                                    <option
                                        @selected(old('stripe_country', config('payment.stripe_country')) == $code)
                                        value="{{ $code }}">
                                        {{ $country }}
                                    </option>
                                @endforeach
                            ...
                            <select name="stripe_currency" id="stripe_currency" class="form-control select2">
                                @foreach (config('currency.currency_list') as $key => $currency)
                                    <option value="{{ $currency }}"
                                        @selected(old('stripe_currency', config('payment.stripe_currency')) == $currency)>
                                        {{ $key }} ({{ $currency }})
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            <label for="stripe_currency_rate">Stripe Currency Rate (Per{{ config('settings.site_default_currency') }})</label>
